...
var (
	...
	ErrInvalidName        = util.NewInvalidArgumentErrorf("service name is invalid")
	ErrInvalidVersion     = util.NewInvalidArgumentErrorf("service version is invalid")
)

const (
	PropertyMetadata     = "awslambda.metadata"
	PropertyBranch       = "awslambda.branch"
	PropertyRepository   = "awslambda.repository"
	PropertyArchitecture = "awslambda.architecture"

	SettingKeyPrivate = "awslambda.key.private"
	SettingKeyPublic  = "awslambda.key.public"

	RepositoryService = "_awslambda"
...
// Service represents an AwsLambda service
type Service struct {
	Name            string
	Version         string
...
// Metadata of an AwsLambda service
...
// ParseService parses the AwsLambda service file
func ParseService(r io.Reader) (*Service, error) {
	// AwsLambda services are concated .tar.gz streams. Usually the first stream contains the service metadata.
...
// ParseServiceInfo parses a PKGINFO file to retrieve the metadata of an AwsLambda service
func ParseServiceInfo(r io.Reader) (*Service, error) {
	p := &Service{}
