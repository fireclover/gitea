...
// ErrDuplicateServiceVersion indicates a duplicated package version error
var ErrDuplicateServiceVersion = util.NewAlreadyExistErrorf("package version already exists")

func init() {
	db.RegisterModel(new(ServiceVersion))
}

// ServiceVersion represents a package version
type ServiceVersion struct {
	ID            int64              `xorm:"pk autoincr"`
	ServiceID     int64              `xorm:"UNIQUE(s) INDEX NOT NULL"`
...
// GetOrInsertVersion inserts a version. If the same version exist already ErrDuplicateServiceVersion is returned
func GetOrInsertVersion(ctx context.Context, pv *ServiceVersion) (*ServiceVersion, error) {
	e := db.GetEngine(ctx)

	existing := &ServiceVersion{}

	has, err := e.Where(builder.Eq{
		"package_id":    pv.ServiceID,
	...
	if has {
		return existing, ErrDuplicateServiceVersion
	}
...
func UpdateVersion(ctx context.Context, pv *ServiceVersion) error {
...
func GetVersionByID(ctx context.Context, versionID int64) (*ServiceVersion, error) {
	pv := &ServiceVersion{}
...
func GetVersionByNameAndVersion(ctx context.Context, ownerID int64, packageType Type, name, version string) (*ServiceVersion, error) {
...
func GetInternalVersionByNameAndVersion(ctx context.Context, ownerID int64, packageType Type, name, version string) (*ServiceVersion, error) {
	return getVersionByNameAndVersion(ctx, ownerID, packageType, name, version, true)
}

func getVersionByNameAndVersion(ctx context.Context, ownerID int64, packageType Type, name, version string, isInternal bool) (*ServiceVersion, error) {
	pvs, _, err := SearchVersions(ctx, &ServiceSearchOptions{
		OwnerID: ownerID,
		Type:    packageType,
		Name: SearchValue{
			ExactMatch: true,
...
// GetVersionsByServiceType gets all versions of a specific type
func GetVersionsByServiceType(ctx context.Context, ownerID int64, packageType Type) ([]*ServiceVersion, error) {
	pvs, _, err := SearchVersions(ctx, &ServiceSearchOptions{
		OwnerID:    ownerID,
		Type:       packageType,
...
// GetVersionsByServiceName gets all versions of a specific package
func GetVersionsByServiceName(ctx context.Context, ownerID int64, packageType Type, name string) ([]*ServiceVersion, error) {
	pvs, _, err := SearchVersions(ctx, &ServiceSearchOptions{
		OwnerID: ownerID,
		Type:    packageType,
		Name: SearchValue{
...
func DeleteVersionByID(ctx context.Context, versionID int64) error {
	_, err := db.GetEngine(ctx).ID(versionID).Delete(&ServiceVersion{})
	return err
}
...
func HasVersionFileReferences(ctx context.Context, versionID int64) (bool, error) {
	return db.GetEngine(ctx).Get(&ServiceFile{
		VersionID: versionID,
	})
}
...
// ServiceSearchOptions are options for SearchXXX methods
...
func (opts *ServiceSearchOptions) ToConds() builder.Cond {
	...
	if opts.ServiceID != 0 {
		cond = cond.And(builder.Eq{"package.id": opts.ServiceID})
	}
	if opts.Name.Value != "" {
...
func (opts *ServiceSearchOptions) configureOrderBy(e db.Engine) {
	switch opts.Sort {
	case SortNameAsc:
		e.Asc("package.name")
	case SortNameDesc:
...
// SearchVersions gets all versions of services matching the search options
func SearchVersions(ctx context.Context, opts *ServiceSearchOptions) ([]*ServiceVersion, int64, error) {
	...
	pvs := make([]*ServiceVersion, 0, 10)
	count, err := sess.FindAndCount(&pvs)
	return pvs, count, err
}

// SearchLatestVersions gets the latest version of every package matching the search options
func SearchLatestVersions(ctx context.Context, opts *ServiceSearchOptions) ([]*ServiceVersion, int64, error) {
	in := builder.
...
func ExistVersion(ctx context.Context, opts *ServiceSearchOptions) (bool, error) {
	return db.GetEngine(ctx).
		Where(opts.ToConds()).
		Table("package_version").
		Join("INNER", "package", "package.id = package_version.package_id").
		Exist(new(ServiceVersion))
}

// CountVersions counts all versions of services matching the search options
func CountVersions(ctx context.Context, opts *ServiceSearchOptions) (int64, error) {
	return db.GetEngine(ctx).
		Where(opts.ToConds()).
		Table("package_version").
		Join("INNER", "package", "package.id = package_version.package_id").
		Count(new(ServiceVersion))
}
